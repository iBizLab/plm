...
export class AvatarUploadController extends EditorController<IFileUploader> {
  ...
  protected async onInit(): Promise<void> {
    ...
    if (this.editorParams) {
      const { isDrag, accept, uploadParams, exportParams } = this.editorParams;
      if (isDrag) {
  ...
  async createCropImageUrl(
    ...
    // 计算要截取的区域的左上角坐标和宽度高度
    const left = x - radius; // 区域左上角 x 坐标
    const top = y - radius; // 区域左上角 y 坐标
    const width = radius * 2; // 区域宽度
    const height = radius * 2; // 区域高度
