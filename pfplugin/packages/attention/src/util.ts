...
export class Util {
  ...
  public static calcSelItemRenderNum(values: IData, el: HTMLElement): number {
    ...
    if (el && values?.length > 0) {
      ...
      // 关注选择触发 Popover 显示的 HTML 元素显示宽度
      const followWidth = 70;
      // 选择人员触发 Popover 显示的 HTML 元素显示宽度
      const selectWidth = 33;
      // 单个选中项元素宽度
      ...
      const computeValue =
        totalWidth - totalMargin - followWidth - selectWidth - selectsWidth;
      // 判断宽度是否会溢出
      if (computeValue < 0) {
        selRenderNum = Math.floor(
          (totalWidth - totalMargin - followWidth - selectWidth) /
