...
export class AttentionController extends EditorController<IPicker> {
   ...
   * 启用确认按钮
   */
  public enableBottomButton: boolean = false;
  ...
  public initParams(): void {
    ...
    if (this.editorParams?.ENABLEBOTTOMBUTTON) {
      this.enableBottomButton =
        this.editorParams?.ENABLEBOTTOMBUTTON === 'true';
    }
    if (this.editorParams?.FIELDMAP) {
      this.fieldMap = this.editorParams?.FIELDMAP;
    }
