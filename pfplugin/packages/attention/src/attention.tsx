...
export const Attention = defineComponent({
  ...
  setup(props, { emit, attrs }) {
    ...
    const multipleSelectClone: Ref<IData[]> = ref([]);
     ...
     * 刷新OBJECTS值
     */
    const refreshObjectsValue = (newVal: IData) => {
      if (newVal) {
      ...
      multipleSelectClone.value = clone(multipleSelect.value);
    };
      ...
      newVal => {
        if (newVal) {
          if (c.valueType === 'OBJECT') {
            ...
            refreshObjectsValue(newVal as IData);
            handleSelectUIShow();
          } else {
    ...
    // 处理额外抛值映射字段
    const handleExtraValue = (select: IData): IData => {
      const item = {};
      const findItem =
        multipleSelectClone.value.find((tempItem: IData) => {
          return tempItem.id === select.id;
        }) || {};
      try {
        const strVal = c.dynamicMatch(c.fieldMap, {
          item: { ...findItem, ...select },
        });
        const objVal = JSON.parse(strVal);
        // 剔除数据为空的字段，适配后台逻辑
        Object.keys(objVal).forEach((key: string) => {
          if (objVal[key]) {
            Object.assign(item, { [key]: objVal[key] });
          }
        });
      } catch (err: unknown) {
        ibiz.log.debug(`${c.fieldMap}额外抛值字段映射`);
      }
      return item;
    };
    ...
    const handleObjectsEmit = (): void => {
      if (c.enableBottomButton && !isConfirm.value) {
        return;
      }
      const values = multipleSelect.value.map((select: IData) => {
        ...
        if (c.fieldMap) {
          Object.assign(tempItem, handleExtraValue(select));
        }
        return tempItem;
      });
      emit('change', values);
    ...
    // 处理弹框底部按钮取消
    const onBottomCancel = () => {
      isConfirm.value = false;
      modelVisible.value = false;
      setTimeout(() => {
        refreshObjectsValue(props.value as IData);
      }, 200);
    };

    // 处理弹框底部按钮确认
    const onBottomConfirm = () => {
      isConfirm.value = true;
      modelVisible.value = false;
      handleObjectsEmit();
      isConfirm.value = false;
    ...
    const renderSelectIcon = (key: string): JSX.Element | void | null => {
      if (c.multiple) {
        ...
        if (refValue.value) {
          const values = refValue.value.split(c.separator);
          index = values.findIndex((value: string) => {
            return value === key;
          });
        }
        if (c.enableBottomButton) {
          index = multipleSelect.value.findIndex((item: IData) => {
            const userid: string =
    ...
    const renderBottomBtn = () => {
      return (
        <div class={ns.be('select-modal', 'bottom')}>
          <el-button type='text' onClick={onBottomCancel}>
            {ibiz.i18n.t('app.cancel')}
          </el-button>
          <el-button onClick={onBottomConfirm}>
    ...
    const renderDropModal = (): JSX.Element => {
      return (
        ...
        >
          ...
          {c.enableBottomButton && renderBottomBtn()}
        </div>
      );
    };
    ...
    const renderSelects = (): JSX.Element | void => {
      if (c.multiple) {
        ...
        if (selRenderNum > 0) {
          isEquality = c.enableBottomButton
            ? selRenderNum < multipleSelectClone.value?.length
            : selRenderNum < multipleSelect.value?.length;
          // 根据计算出的绘制个数绘制项
      ...
      if (c.multiple) {
        return (
          <div class={ns.b('select-value-multiple')}>
            <div class={ns.b('select-value')} title='选择人员'>
              <svg
                viewBox='0 0 16 16'
                xmlns='http://www.w3.org/2000/svg'
                height='1em'
