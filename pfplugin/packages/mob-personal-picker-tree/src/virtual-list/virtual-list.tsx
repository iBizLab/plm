...
export const VirtualList = defineComponent({
  ...
  emits: ['scrollEvent'],
  setup(props, { emit }) {
    // 修改此处样式名称,避免与人员选择插件样式冲突
    const ns = useNamespace('virtual-list-box');
    ...
    // 抛出滚动事件
    const scrollEvent = (height: number, tempScrollTop: number): void => {
      emit('scrollEvent', height, tempScrollTop);
    };
    // 计算可视区域数据
    ...
    const contentHeight = computed(() => {
      const itemHeight = props.itemHeight < 36 ? 36 : props.itemHeight;
