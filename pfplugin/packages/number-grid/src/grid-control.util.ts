...
export function useITableEvent(c: GridController): {
  ...
  const treeNodeData = computed(() => {
    return c.state.items.map(item => {
      item._hasChildren = c.state.items.some(
        child =>
          item[(c as NumberTreeGridController).treeGridValueField] &&
          item[(c as NumberTreeGridController).treeGridValueField] ===
            child[(c as NumberTreeGridController).treeGridParentField],
      );
      item._children = [];
      return item;
    });
  ...
  const treeGirdData = computed(() => {
    const keys: string[] = c.state.items.map(
      item => item[(c as NumberTreeGridController).treeGridValueField],
    );
    return treeNodeData.value.filter(
      item =>
        !item[(c as NumberTreeGridController).treeGridParentField] ||
        !keys.includes(
          item[(c as NumberTreeGridController).treeGridParentField],
        ),
    );
...
export function useAppGridBase(
  ...
  const headerDragend = (
    ...
    // 列宽持久化
    if (columnState) {
      (columnState as IData).columnWidth = newWidth;
      c.saveColumnStates();
    }
  };
