...
export class MobMDCtrlTreeController extends MDCtrlController {
  ...
  protected async onCreated(): Promise<void> {
    ...
    try {
      if (this.model.controlParam?.ctrlParams) {
        const ctrlParams = this.model.controlParam.ctrlParams;
        this.valueField = ctrlParams.VALUEFIELD?.toLowerCase();
        this.parentField = ctrlParams.PARENTFIELD?.toLowerCase();
        if (ctrlParams.ONLYEXPAND) {
          this.onlyExpand = JSON.parse(ctrlParams.ONLYEXPAND);
        }
      }
    } catch (error) {
  ...
  calcNodes(): void {
    if (this.valueField && this.parentField) {
      ...
      const setNodeChildren = (nodes: INodeData[]): void => {
        nodes.forEach(node => {
   ...
   * @memberof MobMDCtrlTreeController
   */
  async onRowClick(data: IData, event?: MouseEvent): Promise<void> {
    const keys = Object.keys(this.onlyExpand);
    const validate =
      keys.length > 0
        ? keys.every(
            key =>
              Object.prototype.hasOwnProperty.call(data, key) &&
              data[key] === this.onlyExpand[key],
          )
        : false;
    if (validate) {
      data._expand = !data._expand;
    } else {
      await super.onRowClick(data, event);
    }
  }
}
