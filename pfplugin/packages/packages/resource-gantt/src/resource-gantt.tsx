...
export const ResourceGanttControl = defineComponent({
  ...
  setup(props) {
    ...
    // 是否相等状态
    const isGanttWidthEq = ref(false);
    // 计算甘特容器与内容区宽度是否相等
    const calcGanttWidthEq = (): boolean => {
      if (ganttBoxRef.value) {
        const { $el } = ganttBoxRef.value;
        const ganttScroll = $el.querySelector('.xg-gantt-container');
        const ganttBody = $el.querySelector('.xg-gantt-body');
        const containerWidth = ganttScroll.offsetWidth ;
    ...
    const observer: ResizeObserver | null = new ResizeObserver(() => {
      // 检查是否有子节点变化，通常表明元素已经渲染
      if (ganttBoxRef.value && ganttBoxRef.value.$el.offsetWidth !== 0) {
        isGanttWidthEq.value = calcGanttWidthEq();
      }
    });

    watch(ganttBoxRef, newVal => {
      if (newVal && newVal.$el) {
        const ganttBody = newVal.$el.querySelector('.xg-gantt-body');
    ...
    const onSliderMove = (sliders: IData[]) => {
      ...
      c._evt.emit('onSliderMove', nodeData);
    ...
    return (
      <iBizControlBase
        ...
        class={[
          ...
          this.ns.is('gantt-width-eq', this.isGanttWidthEq),
        ]}
