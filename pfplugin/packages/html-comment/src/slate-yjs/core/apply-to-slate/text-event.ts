...
import { Delta, Operation } from '../types';
    ...
    if ('attributes' in change && 'retain' in change) {
      ...
      for (
        ...
        const properties = pick(
          node,
          ...(Object.keys(change.attributes) as Array<keyof SlateElement>),
        );
    ...
    if ('insert' in change) {
      ...
      if (SlateText.isText(child)) {
        ...
        if (
          ...
          SlatePath.equals(childPath, lastPath)
        ) {
          return ops.push({
            type: 'insert_text',
            offset: textOffset,
...
export function translateYTextEvent(
  ...
  const targetElement = SlateNode.get(editor, slatePath);

  if (SlateText.isText(targetElement)) {
