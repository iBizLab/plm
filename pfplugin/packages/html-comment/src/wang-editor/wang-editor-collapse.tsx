...
import { Srfuf } from '@ibiz-template/runtime';
...
import { CursorOverlay, SlateYjs } from '../slate-yjs';
...
const IBizHtmlCollapse = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const slateYjs: SlateYjs = new SlateYjs(c.enableRealtime);
    ...
    const handleCreated = async (editor: IDomEditor) => {
      ...
      await c.onCreated(editorRef.value, props.data, toolbarConfig);
    ...
    const calcHtmlStyle = () => {
      awaitTimeout(0, () => {
        if (htmlContent.value && toolbarRef.value) {
          const htmlContentHeight = htmlContent.value.$el.offsetHeight;
    ...
    const renderEditorContent = () => {
      ...
      return (
        <CursorOverlay
          ...
          v-loading={!slateYjs.inited.value}
