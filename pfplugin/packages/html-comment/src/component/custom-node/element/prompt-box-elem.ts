...
import { PromptBoxSelect } from '../components';

/**
 * 提示框元素定义
 */
export class PromptBoxElem extends HTMLElement {
  ...
  async attributeChangedCallback(
    ...
    if (tag === 'data-value') {
      ...
      const handleChange = (detail: IData, param: IParams): void => {
        // 在这里处理日期选择变化后的逻辑
        // 创建一个自定义事件
        const event = new CustomEvent('promptBoxElemEvent', {
      ...
      this.vueApp?.unmount();
      this.innerHTML = '';
      this.vueApp = createApp(PromptBoxSelect, {
        data: value,
      ...
      this.vueApp.component(ElInput.name!, ElInput);
