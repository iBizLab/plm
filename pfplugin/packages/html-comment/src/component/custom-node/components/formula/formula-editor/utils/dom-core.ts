...
export class DomElement<T extends DomElementSelector = DomElementSelector> {
   ...
   */
  forEach(
    fn: (ele: HTMLElement, index?: number) => boolean | unknown,
  ): DomElement {
  ...
  on(type: string, selector: string | Function, fn?: Function): DomElement {
    ...
    return this.forEach(elem => {
      ...
      // 有事件代理
      const agentFn: listener = function (e: Event) {
  ...
  hasClass(className?: string): boolean {
    ...
    const arr: string[] = elem.className.split(/\s/);
  ...
  getNodeTop(editor: Editor): DomElement {
    ...
    if (
      editor.$textLatexElem.equal(this) ||
      editor.$textLatexElem.equal($parent)
    ) {
      return this;
    }
  ...
  renderFormula(): Promise<void> {
    return MathJax.renderFormula(this.elems);
  }
}
