## 生成最近访问 <!-- {docsify-ignore-all} -->
   ...
   在用户对项目数据进行了get或update操作时生成相应的访问记录.type为3，不在界面上进行显示
...
2. 将`Default(传入变量).TOPIC_IDENTIFIER(话题标识)` 设置给  `recent.RECENT_PARENT_IDENTIFIER(访问父类编号)`
3. 将`Default(传入变量).TOPIC_NAME(话题)` 设置给  `recent.RECENT_PARENT_NAME(访问父类名称)`
