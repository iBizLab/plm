...
hide empty description
state "开始" as Begin <<start>> [[$./like#begin {"开始"}]]
state "准备点赞参数" as PREPAREPARAM_01  [[$./like#prepareparam_01 {"准备点赞参数"}]]
state "生成点赞记录" as DEACTION_01  [[$./like#deaction_01 {"生成点赞记录"}]]
state "准备参数" as PREPAREPARAM_02  [[$./like#prepareparam_02 {"准备参数"}]]
state "取消点赞或点踩" as DELOGIC_01  [[$./like#delogic_01 {"取消点赞或点踩"}]]
state "结束" as END_01 <<end>> [[$./like#end_01 {"结束"}]]
state "获取消息信息" as DEACTION_02  [[$./like#deaction_02 {"获取消息信息"}]]


Begin --> PREPAREPARAM_02
PREPAREPARAM_02 --> DELOGIC_01
DELOGIC_01 --> PREPAREPARAM_01 : [[$./like#delogic_01-prepareparam_01{连接名称} 连接名称]]
PREPAREPARAM_01 --> DEACTION_01
DEACTION_01 --> END_01
DELOGIC_01 --> END_01 : [[$./like#delogic_01-end_01{连接名称} 连接名称]]
...
1. 将`like` 设置给  `ai_agent_feedback.FEEDBACK_TYPE(反馈类型)`
2. 将`Default(传入变量).ID(标识)` 设置给  `ai_agent_feedback.MESSAGE_ID(消息标识)`
3. 将`用户全局对象.srfpersonid` 设置给  `ai_agent_feedback.USER_ID(用户标识)`

#### 生成点赞记录 :id=DEACTION_01<sup class="footnote-symbol"> <font color=gray size=1>[实体行为]</font></sup>
...
`message(message).IS_LIKE(是否点赞)` NOTEQ `1`
#### 连接名称 :id=DELOGIC_01-END_01

`message(message).IS_LIKE(是否点赞)` EQ `1`
