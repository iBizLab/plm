...
hide empty description
state "开始" as Begin <<start>> [[$./remove_space_scheduled#begin {"开始"}]]
state "准备参数" as PREPAREPARAM_01  [[$./remove_space_scheduled#prepareparam_01 {"准备参数"}]]
state "过滤空间对应计划" as DEDATASET_01  [[$./remove_space_scheduled#dedataset_01 {"过滤空间对应计划"}]]
state "结束" as END_01 <<end>> [[$./remove_space_scheduled#end_01 {"结束"}]]
state "循环子调用" as LOOPSUBCALL_01  [[$./remove_space_scheduled#loopsubcall_01 {"循环子调用"}]] #green {
state "删除空间执行计划" as DEACTION_01  [[$./remove_space_scheduled#deaction_01 {"删除空间执行计划"}]]
}


Begin --> PREPAREPARAM_01
PREPAREPARAM_01 --> DEDATASET_01
DEDATASET_01 --> LOOPSUBCALL_01 : [[$./remove_space_scheduled#dedataset_01-loopsubcall_01{连接名称} 连接名称]]
...
#### 删除空间执行计划 :id=DEACTION_01<sup class="footnote-symbol"> <font color=gray size=1>[实体行为]</font></sup>
