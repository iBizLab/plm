...
1. 将`for_temp_obj(循环临时变量).ID(标识)` 设置给  `update_member_info(更新成员信息).USER_ID(标识)`
2. 将`admin` 设置给  `update_member_info(更新成员信息).ROLE_ID(角色)`
3. 将`Default(传入变量).ID(知识库标识)` 设置给  `update_member_info(更新成员信息).KB_ID(知识库标识)`

#### 更新管理员身份 :id=DEACTION1<sup class="footnote-symbol"> <font color=gray size=1>[实体行为]</font></sup>



调用实体 [知识库成员(AI_KB_MEMBER)](module/ai/ai_kb_member.md) 行为 [Save](module/ai/ai_kb_member#行为) ，行为参数为`update_member_info(更新成员信息)`
...
|更新成员信息|update_member_info|数据对象|[知识库成员(AI_KB_MEMBER)](module/ai/ai_kb_member.md)||
