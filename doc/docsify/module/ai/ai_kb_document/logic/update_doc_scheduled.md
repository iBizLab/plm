...
hide empty description
state "开始" as Begin <<start>> [[$./update_doc_scheduled#begin {"开始"}]]
state "准备参数" as PREPAREPARAM_01  [[$./update_doc_scheduled#prepareparam_01 {"准备参数"}]]
state "过滤文档对应计划" as DEDATASET_01  [[$./update_doc_scheduled#dedataset_01 {"过滤文档对应计划"}]]
state "结束" as END_01 <<end>> [[$./update_doc_scheduled#end_01 {"结束"}]]
state "循环子调用" as LOOPSUBCALL_01  [[$./update_doc_scheduled#loopsubcall_01 {"循环子调用"}]] #green {
state "构造定时器策略" as RAWSFCODE_01  [[$./update_doc_scheduled#rawsfcode_01 {"构造定时器策略"}]]
state "更新文档执行计划" as DEACTION_01  [[$./update_doc_scheduled#deaction_01 {"更新文档执行计划"}]]
}


Begin --> PREPAREPARAM_01 : [[$./update_doc_scheduled#begin-prepareparam_01{连接名称} 连接名称]]
PREPAREPARAM_01 --> DEDATASET_01
DEDATASET_01 --> LOOPSUBCALL_01 : [[$./update_doc_scheduled#dedataset_01-loopsubcall_01{连接名称} 连接名称]]
...
2. 将`Default(传入变量).ID(知识库文档标识)` 设置给  `extend_schedule_filter.n_principal_id_eq`

#### 过滤文档对应计划 :id=DEDATASET_01<sup class="footnote-symbol"> <font color=gray size=1>[实体数据集]</font></sup>
...
#### 更新文档执行计划 :id=DEACTION_01<sup class="footnote-symbol"> <font color=gray size=1>[实体行为]</font></sup>
