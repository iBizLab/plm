## 生成最近访问 <!-- {docsify-ignore-all} -->
   ...
   在用户对空间数据进行了get或update操作时生成相应的访问记录
...
2. 将`Default(传入变量).IDENTIFIER(空间标识)` 设置给  `recent.RECENT_PARENT_IDENTIFIER(访问父类编号)`
3. 将`Default(传入变量).NAME(空间名称)` 设置给  `recent.RECENT_PARENT_NAME(访问父类名称)`
...
7. 将`Default(传入变量).NAME(空间名称)` 设置给  `recent.NAME(名称)`
8. 将`space` 设置给  `recent.OWNER_TYPE(所属数据对象)`
9. 将`space` 设置给  `recent.OWNER_SUBTYPE(所属对象子类型)`
