## 当前模块下用例(normal) <!-- {docsify-ignore-all} -->
...
((`SUITE_ID(用例模块标识)` EQ `网页请求上下文.suite_id` OR `SUITES(模块路径)` LIKE `网页请求上下文.suite_id`))
...
WHERE ( ( <choose><when test="ctx.webcontext.suite_id !=null ">  t11.SUITE_ID = #{ctx.webcontext.suite_id}  </when><otherwise>1=1</otherwise></choose>  OR  t31.SUITES LIKE #{ctx.webcontext.suite_id} ) )
```
