## 返回（历史） <!-- {docsify-ignore-all} -->

   查看评审历史后，返回测试用例主表单按钮使用
...
state "开始" as Begin <<start>> [[$./back_history#begin {开始}]]
state "切换显示组件" as RAWJSCODE1  [[$./back_history#rawjscode1 {切换显示组件}]]
state "获取父视图对象" as PREPAREJSPARAM1  [[$./back_history#preparejsparam1 {获取父视图对象}]]
state "表单刷新" as VIEWCTRLINVOKE1  [[$./back_history#viewctrlinvoke1 {表单刷新}]]
state "绑定表单" as PREPAREJSPARAM2  [[$./back_history#preparejsparam2 {绑定表单}]]
