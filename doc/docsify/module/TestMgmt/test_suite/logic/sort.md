...
2. 将`Default(传入变量).OWNER_ID` 设置给  `filter.N_LIBRARY_ID_EQ`

#### 实体数据集 :id=DEDATASET1<sup class="footnote-symbol"> <font color=gray size=1>[实体数据集]</font></sup>



调用实体 [用例模块(TEST_SUITE)](module/TestMgmt/test_suite.md) 数据集合 [数据集(DEFAULT)](module/TestMgmt/test_suite#数据集合) ，查询参数为`filter`
...
|    中文名   |    代码名    |  数据类型    |  实体   |备注 |
| --------| --------| -------- | -------- | --------   |
|传入变量(<i class="fa fa-check"/></i>)|Default|数据对象|[用例模块(TEST_SUITE)](module/TestMgmt/test_suite.md)||
|filter|filter|过滤器|||
|page|page|分页查询|||
