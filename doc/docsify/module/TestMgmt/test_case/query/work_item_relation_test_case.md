...
连接条件：((`PRINCIPAL_TYPE(关联主体类型)` EQ `'work_item'` OR `PRINCIPAL_TYPE(关联主体类型)` EQ `'bug'`) AND `TARGET_TYPE(关联目标类型)` EQ `'test_case'` AND `PRINCIPAL_ID(关联主体标识)` EQ `网页请求上下文.principal_id`)<br>
