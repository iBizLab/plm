## 工时登记用例(workload_case) <!-- {docsify-ignore-all} -->
...
(`IS_ARCHIVED(是否已归档)` EQ `'0'` AND `IS_DELETED(是否已删除)` EQ `'0'` AND `LIBRARY_IS_ARCHIVED(测试库是否归档)` EQ `'0'` AND `LIBRARY_IS_DELETED(测试库是否删除)` EQ `'0'`)
