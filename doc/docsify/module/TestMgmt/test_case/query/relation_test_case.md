## 关联测试用例(relation_test_case) <!-- {docsify-ignore-all} -->
...
连接条件：(`PRINCIPAL_ID(关联主体标识)` EQ `网页请求上下文.principal_id` AND `PRINCIPAL_TYPE(关联主体类型)` EQ `'idea'` AND `TARGET_TYPE(关联目标类型)` EQ `'test_case'`)<br>
