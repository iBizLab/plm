## 关联测试用例（多需求）(ideas_relation_test_case) <!-- {docsify-ignore-all} -->
...
### 查询连接
* **RELATION左外联接 1:NDERCUSTOM_RELATION_TARGET_TEST_CASE**<br>
连接关系：[DERCUSTOM_RELATION_TARGET_TEST_CASE](der/DERCUSTOM_RELATION_TARGET_TEST_CASE)<br>
连接实体：[用例](module/TestMgmt/test_case)<br>
连接条件：(`PRINCIPAL_TYPE(关联主体类型)` EQ `'idea'` AND `TARGET_TYPE(关联目标类型)` EQ `'test_case'` AND ` t31.`PRINCIPAL_ID` IN (${ctx.datacontext.principal_id})`)<br>
