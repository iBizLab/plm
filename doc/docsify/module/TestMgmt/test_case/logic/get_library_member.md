...
state "空" as PREPAREPARAM3  [[$./get_library_member#prepareparam3 {"空"}]]
...
Begin --> PREPAREPARAM3
PREPAREPARAM3 --> RAWSFCODE4 : [[$./get_library_member#prepareparam3-rawsfcode4{已删除已归档} 已删除已归档]]
RAWSFCODE4 --> END2
PREPAREPARAM3 --> RAWSFCODE3 : [[$./get_library_member#prepareparam3-rawsfcode3{连接名称} 连接名称]]
...
(`Default(传入变量).IS_ARCHIVED(是否已归档)` EQ `1` OR `Default(传入变量).IS_DELETED(是否已删除)` EQ `1` OR `Default(传入变量).library_is_archived(测试库是否归档)` EQ `1`)
