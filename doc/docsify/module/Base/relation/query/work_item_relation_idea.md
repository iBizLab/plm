## 工作项关联产品需求(work_item_relation_idea) <!-- {docsify-ignore-all} -->
...
(`PRINCIPAL_ID(关联主体标识)` EQ `网页请求上下文.principal_id` AND `PRINCIPAL_TYPE(关联主体类型)` EQ `'work_item'` AND `TARGET_TYPE(关联目标类型)` EQ `'idea'`)



### 查询连接
* **IDEA相关N:1（INNER JOIN）DERCUSTOM_RELATION_TARGET_IDEA**<br>
