## 工单关联产品需求(ticket_re_idea) <!-- {docsify-ignore-all} -->
...
(`PRINCIPAL_ID(关联主体标识)` EQ `网页请求上下文.principal_id` AND `PRINCIPAL_TYPE(关联主体类型)` EQ `'ticket'` AND `TARGET_TYPE(关联目标类型)` EQ `'idea'`)
