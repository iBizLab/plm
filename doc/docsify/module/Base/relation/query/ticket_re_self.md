## 工单关联工单(ticket_re_self) <!-- {docsify-ignore-all} -->
...
(`PRINCIPAL_ID(关联主体标识)` EQ `网页请求上下文.principal_id` AND `TARGET_TYPE(关联目标类型)` EQ `'ticket'` AND `PRINCIPAL_TYPE(关联主体类型)` EQ `'ticket'`)
