## 工作项关联工作项(work_item_relation_self) <!-- {docsify-ignore-all} -->
...
(`PRINCIPAL_ID(关联主体标识)` EQ `网页请求上下文.principal_id` AND `PRINCIPAL_TYPE(关联主体类型)` EQ `'work_item'` AND `TARGET_TYPE(关联目标类型)` EQ `'work_item'`)
