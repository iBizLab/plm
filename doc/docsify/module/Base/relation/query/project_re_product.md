## 项目关联产品(project_re_product) <!-- {docsify-ignore-all} -->
...
(`PRINCIPAL_ID(关联主体标识)` EQ `网页请求上下文.principal_id` AND `TARGET_TYPE(关联目标类型)` EQ `'product'` AND `PRINCIPAL_TYPE(关联主体类型)` EQ `'project'`)



### 查询连接
* **PRODUCT相关N:1（INNER JOIN）DERCUSTOM_RELATION_TARGET_PRODUCT**<br>
