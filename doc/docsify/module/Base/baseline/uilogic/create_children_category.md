...
state "准备参数" as PREPAREJSPARAM5  [[$./create_children_category#preparejsparam5 {准备参数}]]
...
state "准备参数" as PREPAREJSPARAM7  [[$./create_children_category#preparejsparam7 {准备参数}]]
state "准备参数" as PREPAREJSPARAM4  [[$./create_children_category#preparejsparam4 {准备参数}]]


Begin --> PREPAREJSPARAM2 : [[$./create_children_category#begin-preparejsparam2{产品} 产品]]
PREPAREJSPARAM2 --> PREPAREJSPARAM1
PREPAREJSPARAM1 --> VIEWCTRLINVOKE1
Begin --> PREPAREJSPARAM7 : [[$./create_children_category#begin-preparejsparam7{空间} 空间]]
PREPAREJSPARAM7 --> PREPAREJSPARAM1
Begin --> PREPAREJSPARAM4 : [[$./create_children_category#begin-preparejsparam4{测试库} 测试库]]
PREPAREJSPARAM4 --> PREPAREJSPARAM1
Begin --> PREPAREJSPARAM5 : [[$./create_children_category#begin-preparejsparam5{项目} 项目]]
PREPAREJSPARAM5 --> PREPAREJSPARAM1
...
1. 将`product` 设置给  `defaultValue(默认值对象).owner_type`
2. 将`context(上下文对象).product` 设置给  `defaultValue(默认值对象).owner_id`

#### 准备参数 :id=PREPAREJSPARAM5<sup class="footnote-symbol"> <font color=gray size=1>[准备参数]</font></sup>



1. 将`project` 设置给  `defaultValue(默认值对象).owner_type`
...
1. 将`library` 设置给  `defaultValue(默认值对象).owner_type`
2. 将`context(上下文对象).library` 设置给  `defaultValue(默认值对象).owner_id`

#### 准备参数 :id=PREPAREJSPARAM7<sup class="footnote-symbol"> <font color=gray size=1>[准备参数]</font></sup>



1. 将`space` 设置给  `defaultValue(默认值对象).owner_type`
2. 将`context(上下文对象).space` 设置给  `defaultValue(默认值对象).owner_id`

### 连接条件说明
#### 产品 :id=Begin-PREPAREJSPARAM2

```context(上下文对象).product``` ISNOTNULL AND ```context(上下文对象).product``` NOTEQ ```undefined```
#### 空间 :id=Begin-PREPAREJSPARAM7

```context(上下文对象).space``` ISNOTNULL AND ```context(上下文对象).space``` NOTEQ ```undefined```
#### 测试库 :id=Begin-PREPAREJSPARAM4

```context(上下文对象).library``` ISNOTNULL AND ```context(上下文对象).library``` NOTEQ ```undefined```
#### 项目 :id=Begin-PREPAREJSPARAM5

```context(上下文对象).project``` ISNOTNULL AND ```context(上下文对象).project``` NOTEQ ```undefined```
