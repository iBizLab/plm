## 我的工作类别记录(my_type_of) <!-- {docsify-ignore-all} -->
...
(`CREATE_MAN(建立人)` EQ `用户上下文.srfpersonid` AND `<choose><when test="ctx.webcontext.type_id !=null ">  t1.`TYPE_ID` = #{ctx.webcontext.type_id}  </when><otherwise>t1.`TYPE_ID` is null</otherwise></choose>`)
