...
4. 将`Default(传入变量).SECTION_ID(分组标识)` 设置给  `filter.N_SECTION_ID_EQ`
5. 将`Default(传入变量).OWNER_ID(所属数据标识)` 设置给  `filter.N_OWNER_ID_EQ`

#### 实体数据集 :id=DEDATASET1<sup class="footnote-symbol"> <font color=gray size=1>[实体数据集]</font></sup>



调用实体 [类别(CATEGORY)](module/Base/category.md) 数据集合 [数据集(DEFAULT)](module/Base/category#数据集合) ，查询参数为`filter`
