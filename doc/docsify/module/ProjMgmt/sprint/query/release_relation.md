## 发布关联迭代(release_relation) <!-- {docsify-ignore-all} -->
...
连接条件：(`PRINCIPAL_TYPE(关联主体类型)` EQ `'sprint'` AND `TARGET_TYPE(关联目标类型)` EQ `'release'` AND `TARGET_ID(关联目标标识)` EQ `网页请求上下文.release`)<br>
