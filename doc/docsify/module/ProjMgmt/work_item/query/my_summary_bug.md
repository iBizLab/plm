## 我的事项_缺陷数(my_summary_bug) <!-- {docsify-ignore-all} -->
...
((`ASSIGNEE_ID(负责人)` EQ `用户上下文.srfpersonid` OR `exists(select 1 from executor t2 where t1.id = t2.owner_id and t2.owner_type = 'WORK_ITEM' and t2.owner_subtype = 'WORK_ITEM' and t2.user_id = #{ctx.sessioncontext.srfpersonid})`) AND `IS_ARCHIVED(是否已归档)` EQ `'0'` AND `IS_DELETED(是否已删除)` EQ `'0'` AND `WORK_ITEM_TYPE_GROUP(工作项类型分组)` EQ `'bug'` AND `STATE_TYPE(工作项状态类型)` EQ `'in_progress'`)
