...
连接条件：(`PRINCIPAL_TYPE(关联主体类型)` EQ `'baseline'` AND `TARGET_TYPE(关联目标类型)` EQ `'work_item'` AND `PRINCIPAL_ID(关联主体标识)` EQ `网页请求上下文.baseline`)<br>
...
WHERE EXISTS(SELECT * FROM `RELATION` t91 
 WHERE 
 t1.`ID` = t91.`TARGET_ID`  AND  ( t91.`PRINCIPAL_TYPE` = 'baseline'  AND  t91.`TARGET_TYPE` = 'work_item'  AND  t91.`PRINCIPAL_ID` = #{ctx.webcontext.baseline} ) )
```
