## 资源分配(resource_assignment) <!-- {docsify-ignore-all} -->
...
(`IS_ARCHIVED(是否已归档)` EQ `'0'` AND `IS_DELETED(是否已删除)` EQ `'0'` AND (`t1.START_AT BETWEEN #{ctx.webcontext.n_date_scope_gtand}
AND #{ctx.webcontext.n_date_scope_ltand}` OR `t1.END_AT BETWEEN #{ctx.webcontext.n_date_scope_gtand}
AND #{ctx.webcontext.n_date_scope_ltand}`) AND `ASSIGNEE_ID(负责人)` ISNOTNULL)
