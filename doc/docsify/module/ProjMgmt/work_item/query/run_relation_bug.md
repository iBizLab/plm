## 执行用例关联缺陷(run_relation_bug) <!-- {docsify-ignore-all} -->
...
连接条件：(`TARGET_TYPE(关联目标类型)` EQ `'bug'` AND `PRINCIPAL_ID(关联主体标识)` EQ `网页请求上下文.run` AND (`PRINCIPAL_TYPE(关联主体类型)` EQ `'run'` OR `PRINCIPAL_TYPE(关联主体类型)` EQ `'test_case'`))<br>
