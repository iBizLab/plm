## 工作项关联工作项(work_item_relation_work_item) <!-- {docsify-ignore-all} -->
...
连接条件：(`TARGET_TYPE(关联目标类型)` EQ `'work_item'` AND `PRINCIPAL_TYPE(关联主体类型)` EQ `'work_item'` AND `PRINCIPAL_ID(关联主体标识)` EQ `网页请求上下文.principal_id`)<br>
