## 工时登记工作项(workload_work_item) <!-- {docsify-ignore-all} -->
...
(`IS_ARCHIVED(是否已归档)` EQ `'0'` AND `IS_DELETED(是否已删除)` EQ `'0'` AND `PROJECT_IS_ARCHIVED(项目是否归档)` EQ `'0'` AND `PROJECT_IS_DELETED(项目是否删除)` EQ `'0'`)
...
连接实体：[项目](module/ProjMgmt/project)<br>
连接条件：((`ROLE_ID(角色)` EQ `'admin'` OR `ROLE_ID(角色)` EQ `'user'`))<br>
