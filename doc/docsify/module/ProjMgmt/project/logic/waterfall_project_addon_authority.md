## waterfall项目组件权限计数器 <!-- {docsify-ignore-all} -->
...
hide empty description
state "开始" as Begin <<start>> [[$./waterfall_project_addon_authority#begin {"开始"}]]
state "准备组件过滤参数" as PREPAREPARAM1  [[$./waterfall_project_addon_authority#prepareparam1 {"准备组件过滤参数"}]]
state "获取组件列表" as DEDATASET1  [[$./waterfall_project_addon_authority#dedataset1 {"获取组件列表"}]]
state "构建计数器结果" as RAWSFCODE1  [[$./waterfall_project_addon_authority#rawsfcode1 {"构建计数器结果"}]]
state "结束" as END1 <<end>> [[$./waterfall_project_addon_authority#end1 {"结束"}]]
...
2. 将`waterfall` 设置给  `filter(过滤器).n_owner_subtype_eq`
