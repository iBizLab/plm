...
4. 将`product_plan` 设置给  `defaultValue(默认值对象).owner_subtype`
5. 将`product` 设置给  `defaultValue(默认值对象).owner_type`
6. 将`context(上下文对象).product` 设置给  `defaultValue(默认值对象).owner_id`
