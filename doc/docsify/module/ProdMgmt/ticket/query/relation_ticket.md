## 关联工单(relation_ticket) <!-- {docsify-ignore-all} -->
...
连接条件：(`TARGET_TYPE(关联目标类型)` EQ `'ticket'` AND `PRINCIPAL_TYPE(关联主体类型)` EQ `'idea'` AND `PRINCIPAL_ID(关联主体标识)` EQ `网页请求上下文.principal_id`)<br>
