...
2. 将`Default(传入变量).IDENTIFIER(产品标识)` 设置给  `recent.RECENT_PARENT_IDENTIFIER(访问父类编号)`
3. 将`Default(传入变量).NAME(产品名称)` 设置给  `recent.RECENT_PARENT_NAME(访问父类名称)`
...
7. 将`Default(传入变量).NAME(产品名称)` 设置给  `recent.NAME(名称)`
8. 将`product` 设置给  `recent.OWNER_TYPE(所属数据对象)`
9. 将`product` 设置给  `recent.OWNER_SUBTYPE(所属对象子类型)`
