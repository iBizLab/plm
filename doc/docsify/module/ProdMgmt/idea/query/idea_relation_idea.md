## 产品需求关联需求(idea_relation_idea) <!-- {docsify-ignore-all} -->
...
连接条件：(`PRINCIPAL_ID(关联主体标识)` EQ `网页请求上下文.principal_id` AND `TARGET_TYPE(关联目标类型)` EQ `'idea'` AND `PRINCIPAL_TYPE(关联主体类型)` EQ `'idea'`)<br>
