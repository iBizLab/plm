## 未添加的需求(not_add_idea) <!-- {docsify-ignore-all} -->
...
(`IS_DELETED(是否已删除)` EQ `'0'` AND `SECTION_ID(子产品标识)` EQ `网页请求上下文.section_id` AND `not exists(select 1 from `relation` t2 where t1.id = t2.target_id and t2.principal_id = #{ctx.webcontext.add_review} and t2.principal_type = 'REVIEW' and t2.target_type = 'IDEA')`)
