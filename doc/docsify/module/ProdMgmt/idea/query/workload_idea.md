## 工时登记需求(workload_idea) <!-- {docsify-ignore-all} -->
...
(`IS_ARCHIVED(是否已归档)` EQ `'0'` AND `IS_DELETED(是否已删除)` EQ `'0'` AND `PRODUCT_IS_DELETED(产品是否删除)` EQ `'0'` AND `PRODUCT_IS_ARCHIVED(产品是否归档)` EQ `'0'`)
