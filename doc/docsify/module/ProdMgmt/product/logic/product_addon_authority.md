...
hide empty description
state "开始" as Begin <<start>> [[$./product_addon_authority#begin {"开始"}]]
state "准备组件过滤参数" as PREPAREPARAM1  [[$./product_addon_authority#prepareparam1 {"准备组件过滤参数"}]]
state "获取组件列表" as DEDATASET1  [[$./product_addon_authority#dedataset1 {"获取组件列表"}]]
state "构建计数器结果" as RAWSFCODE1  [[$./product_addon_authority#rawsfcode1 {"构建计数器结果"}]]
state "结束" as END1 <<end>> [[$./product_addon_authority#end1 {"结束"}]]
...
2. 将`product` 设置给  `filter(过滤器).n_owner_type_eq`
...
|组件集合|addons|分页查询|||
|过滤器|filter|过滤器|||
|计数结果|result|数据对象|[产品(PRODUCT)](module/ProdMgmt/product.md)||
