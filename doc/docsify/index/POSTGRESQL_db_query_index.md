...
WHERE ( t21.SUB_TYPE LIKE #{ctx.webcontext.work_item_type_id} ) AND ( t1.IS_DELETED = 0  AND  t1.IS_ARCHIVED = 0 )
```
...
WHERE ( ( (current_date - t1.CREATE_TIME::date) < 7  OR  (current_date - t1.COMPLETED_AT::date) < 7 )  AND  t1.IS_DELETED = 0 )
```
