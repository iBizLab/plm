...
#### 附件(attachments) :id=attachment-attachments
```sql
SELECT
t1.`CREATE_MAN`,
t1.`CREATE_TIME`,
...
#### 关联需求（多需求）(ideas_relation_idea) :id=idea-ideas_relation_idea
```sql
SELECT
t1.`ASSIGNEE_ID`,
t1.`ASSIGNEE_NAME`,
...
#### 关联测试用例（多需求）(ideas_relation_test_case) :id=test_case-ideas_relation_test_case
```sql
SELECT
t1.`CREATE_MAN`,
t1.`CREATE_TIME`,
...
#### 关联工单（多需求）(ideas_relation_ticket) :id=ticket-ideas_relation_ticket
```sql
SELECT
t1.`ASSIGNEE_ID`,
t1.`ASSIGNEE_NAME`,
...
WHERE ( t31.`PRINCIPAL_TYPE` = 'idea'  AND  t31.`TARGET_TYPE` = 'ticket'  AND  t31.`PRINCIPAL_ID` IN (SELECT
    t1.`ID` 
  FROM
    `IDEA` t1 
  WHERE
...
#### 关联工作项（多需求）(ideas_relation_work_item) :id=work_item-ideas_relation_work_item
```sql
SELECT
t1.`ACTUAL_END_AT`,
t1.`ACTUAL_START_AT`,
...
WHERE ( t101.`PRINCIPAL_TYPE` = 'idea'  AND  t101.`TARGET_TYPE` = 'work_item'  AND  t101.`PRINCIPAL_ID` IN (SELECT
    t1.`ID` 
  FROM
    `IDEA` t1 
  WHERE
