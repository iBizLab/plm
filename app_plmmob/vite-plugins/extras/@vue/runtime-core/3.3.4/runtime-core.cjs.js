...
const KeepAliveImpl = {
  ...
  setup(props, { slots }) {
    ...
    const unCache = /* @__PURE__ */ new Map();
    ...
    function pruneCache(filter) {
      cache.forEach((vnode, key) => {
        const name = props.isKey ? vnode.key : getComponentName(vnode.type);
        if (name && (!filter || !filter(name))) {
          pruneCacheEntry(key);
        }
      });
    }
    function loopPruneCacheEntry(key, count = 0) {
      if (count > 30) {
        return;
      }
      if (unCache.has(key)) {
    ...
    function pruneCacheEntry(key, count) {
      const cached = cache.get(key) || unCache.get(key);
    ...
    return () => {
      ...
      const name = props.isKey ? vnode.key : getComponentName(
